...
{
    //created on ground by ice arrow
    public class IceSpike : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Main.EntitySpriteDraw(t.Value, Projectile.Center - Main.screenPosition, new Rectangle(0, 74 * Projectile.frame, 24, 74), lightColor, Projectile.rotation, new Vector2(t.Width() / 2, originY), new Vector2(Projectile.scale, TCellsUtils.LerpFloat(0, 1, Projectile.ai[1], 1, TCellsUtils.LerpEasing.OutCubic)), SpriteEffects.None);
            return false;
        }
            ...
            if (Projectile.ai[0] == 0)
            {
                ...
                for (int i = -16; i <= 16; i += 16)
                {
                    for (int j = -16; j <= 16; j += 16)
                    {
                        ...
                        if (WorldGen.SolidTile(Main.tile[(pos).ToTileCoordinates()]) && (pos.Distance(Projectile.Center) < tilePos.Distance(Projectile.Center) || Projectile.Center == tilePos))
                        {
                            tilePos = pos;
                        }
                    }
                }
                Projectile.rotation = Projectile.AngleTo(tilePos) + Main.rand.NextFloat(MathHelper.ToRadians(-30), MathHelper.ToRadians(30)) + MathHelper.PiOver2;
                int offX = 0;
                int offY = 0;
