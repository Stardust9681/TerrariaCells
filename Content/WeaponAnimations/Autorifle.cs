...
namespace TerrariaCells.Content.WeaponAnimations
{
    public class Autorifle : Gun
    {
        ...
        public override bool CanUseItem(Item item, Player player)
        {
            ...
            {
                player.HeldItem.useTime = (int)(
                    ContentSamples.ItemsByType[player.HeldItem.type].useTime
                );
                player.HeldItem.useAnimation = (int)(
                    ContentSamples.ItemsByType[player.HeldItem.type].useAnimation
                );
                player.HeldItem.reuseDelay = (int)(
                    ContentSamples.ItemsByType[player.HeldItem.type].reuseDelay
                );
            }
            else
            {
                player.HeldItem.useTime = (int)(
                    ContentSamples.ItemsByType[player.HeldItem.type].useTime * ReloadTimeMult
                );
                player.HeldItem.useAnimation = (int)(
                    ContentSamples.ItemsByType[player.HeldItem.type].useAnimation * ReloadTimeMult
                );
                player.HeldItem.reuseDelay = (int)(
                    ContentSamples.ItemsByType[player.HeldItem.type].reuseDelay * ReloadTimeMult
                );
            }
        ...
        public override void UseStyle(Item item, Player player, Rectangle heldItemFrame)
        {
            ...
            if (Ammo > 0 && !mplayer.reloading && player.altFunctionUse != 2)
            {
                ...
                player.SetCompositeArmBack(
                    ...
                    Player.CompositeArmStretchAmount.Full,
                    player.itemRotation
                        - (MathHelper.PiOver2 - MathHelper.ToRadians(10)) * player.direction
                );
            }
            else
            {
                ...
                if (ReloadStep == 0)
                {
                    Player.CompositeArmStretchAmount stretch = Player
                    ...
                    player.SetCompositeArmFront(
                        true,
                        stretch,
                        player.itemRotation - MathHelper.PiOver2 * mplayer.useDirection
                    );
                }
                else if (ReloadStep == 1)
                {
                    float handRot = TCellsUtils.LerpFloat(
                    ...
                    if (animationTime > player.itemAnimationMax / 2)
                    {
                        handRot = TCellsUtils.LerpFloat(
                            0,
                            -50 * mplayer.useDirection,
                            animationTime - ((player.itemAnimationMax - 4) / 2),
                            player.itemAnimationMax / 2,
                            TCellsUtils.LerpEasing.InSine
                        );
                    }
