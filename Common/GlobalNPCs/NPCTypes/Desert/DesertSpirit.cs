...
namespace TerrariaCells.Common.GlobalNPCs.NPCTypes.Shared
{
	public partial class Casters : GlobalNPC
	{
		...
		public bool DesertSpiritAI(NPC npc, Player? target)
		{
			...
			if (npc.ai[0] == 0)
			{
                npc.ai[0] = (int)npc.position.X / 16;
                npc.ai[1] = (int)(npc.position.Y + npc.height) / 16; // Pack(npc.Center.ToTileCoordinates16());
			...
			*/
            ...
            Vector2 worldPos = new Vector2(npc.ai[0], npc.ai[1]).ToWorldCoordinates();
            ...
            //npc.Center = new Vector2(npc.ai[0], npc.ai[1]).ToWorldCoordinates() + new Vector2((float)Math.Sin(MathHelper.ToRadians(npc.ai[3] * 2)) * 50, (float)Math.Sin(MathHelper.ToRadians(npc.ai[3] * 5)) * 10);
